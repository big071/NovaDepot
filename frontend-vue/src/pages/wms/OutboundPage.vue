<template>
  <section class="space-y-4">
    <header class="nd-hero">
      <div class="nd-hero-header">
        <div>
          ...
          <p class="nd-page-subtitle">支持提交备注、审核意见、驳回原因、撤回原因、作废原因和发运备注，并可复盘完整处理历史。</p>
        </div>
    ...
    <n-modal v-model:show="detailVisible" preset="card" title="出库单详情" class="max-w-6xl">
      <div class="space-y-3">
        ...
        <article class="rounded-xl border border-border bg-bg/60 p-3">
          <p class="mb-2 text-sm font-medium">出库明细</p>
  ...
  { title: "下一步", key: "next", minWidth: 220, render: (row) => nextStep(row.status) },
  {
    ...
    render: (row) =>
      h("div", { class: "flex flex-wrap gap-2" }, [
        ...
        actionBtn("审核通过", () => runAction("approve", row), row.status === "SUBMITTED" && authStore.hasPermission("OUTBOUND_APPROVE"), row.id),
        actionBtn("驳回", () => runAction("reject", row), row.status === "SUBMITTED" && authStore.hasPermission("OUTBOUND_APPROVE"), row.id),
        actionBtn("反审核", () => runAction("unapprove", row), row.status === "APPROVED" && authStore.hasPermission("OUTBOUND_UNAPPROVE"), row.id),
...
function nextStep(status: string) {
  const map: Record<string, string> = {
    ...
    SHIPPED: "流程完成，可到库存页复核"
  };
  return map[status] || "按当前状态继续处理";
}

async function onWarehouseChange() {
  locations.value = createForm.warehouseId ? await wmsApi.listLocations(createForm.warehouseId) : [];
...
async function runAction(action: "submit" | "withdraw" | "approve" | "reject" | "unapprove" | "cancel" | "ship", row: OutboundOrder) {
  if ((action === "approve" || action === "reject" || action === "unapprove") && !authStore.hasPermission("OUTBOUND_APPROVE") && !authStore.hasPermission("OUTBOUND_UNAPPROVE")) {
  ...
  try {
    ...
    if (action === "unapprove") await wmsApi.unapproveOutboundOrder(row.id, note);
