<template>
  <section class="space-y-4">
    <header class="flex flex-wrap items-center justify-between gap-3">
      <div>
        <h1 class="text-2xl font-semibold tracking-tight">出库管理</h1>
        <p class="text-sm text-text-secondary">最小可用：列表 + 新建 + 审核 + 发运。</p>
      </div>
      <n-button type="primary" @click="createVisible = true">新建出库单</n-button>
    </header>
    ...
    <n-modal v-model:show="createVisible" preset="card" title="新建出库单" class="max-w-xl">
      <div class="space-y-3">
        <div class="grid grid-cols-2 gap-3">
          ...
          <n-input-number v-model:value="form.customerId" :show-button="false" placeholder="客户ID（可选）" />
        </div>
...
import { wmsApi, type OutboundOrder } from "@/services/wms";
...
const form = reactive({ warehouseId: 1, customerId: 0, productId: 1, locationId: 1, qty: 1 });

const columns: DataTableColumns<OutboundOrder> = [
  { title: "ID", key: "id", width: 80 },
  { title: "出库单号", key: "outboundNo" },
  { title: "仓库ID", key: "warehouseId" },
  { title: "状态", key: "status", render: (row) => h(NTag, { type: row.status === "SHIPPED" ? "success" : "warning" }, { default: () => row.status }) },
  {
    title: "操作",
    key: "actions",
    render: (row) => h("div", { class: "flex gap-2" }, [
      ...
      h(NButton, { size: "small", type: "primary", loading: actionLoading.value === row.id, onClick: () => runAction("ship", row.id) }, { default: () => "发运" })
    ])
  }
];
...
async function loadList() {
  loading.value = true;
  try {
    rows.value = await wmsApi.listOutboundOrders();
  } catch (error) {
    message.error(error instanceof Error ? error.message : "出库单加载失败");
  } finally {
    loading.value = false;
  }
}
...
async function onCreate() {
  if (!form.warehouseId || !form.productId || !form.locationId || !form.qty || Number(form.qty) <= 0) {
    message.warning("请填写完整且有效的出库单信息");
    return;
  }
  ...
  try {
    ...
    message.error(error instanceof Error ? error.message : "出库单创建失败");
  } finally {
    submitting.value = false;
  }
}

async function runAction(action: "approve" | "ship", id: number) {
  actionLoading.value = id;
  try {
    if (action === "approve") {
      await wmsApi.approveOutboundOrder(id);
      message.success("出库单审核成功");
    } else {
      await wmsApi.shipOutboundOrder(id);
      message.success("出库单发运成功");
    }
    await loadList();
  } catch (error) {
