<template>
  <section class="space-y-5">
    ...
    <DataTable title="往来单位列表" :subtitle="`共 ${rows.length} 条记录`" :columns="columns" :data="tableRows" :loading="loading">
      <template #empty>
        <EmptyState description="暂无往来单位数据。">
          <template #extra>
            <n-button v-if="canWrite" class="nd-soft-focus" type="primary" @click="openCreate">立即创建</n-button>
          </template>
        </EmptyState>
      </template>
    </DataTable>
